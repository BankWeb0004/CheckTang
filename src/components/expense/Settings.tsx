...
export function Settings() {
  ...
  // ---------- Save as Image (pretty summary card) ----------
  const renderBackupImage = (): string => {
    const W = 1080;
    const H = 1920;
    const canvas = document.createElement("canvas");
    canvas.width = W;
    ...
    const accent =
      ...
      "oklch(0.65 0.18 250)";
    // Resolve oklch to hex via temp element
    ...
    // Background gradient (accent → darker)
    const bg = ctx.createLinearGradient(0, 0, 0, H);
    bg.addColorStop(0, rgb);
    bg.addColorStop(1, "#0b0b14");
    ctx.fillStyle = bg;
    ctx.fillRect(0, 0, W, H);

    // Subtle glow blob
    const glow = ctx.createRadialGradient(W * 0.8, 200, 40, W * 0.8, 200, 600);
    ...
    ctx.textAlign = "left";
    ctx.fillText("💰 เช็คตังค์ · CHECK TANG", 80, 130);

    const now = new Date();
    const dateLabel = now.toLocaleDateString(lang === "th" ? "th-TH" : "en-US", {
      day: "numeric",
      month: "long",
      year: "numeric",
    });
    ...
    ctx.fillText(lang === "th" ? "ยอดเงินทั้งหมด" : "Net Worth", W / 2, cardY + 90);

    // Net worth amount
    ctx.fillStyle = "#0f172a";
    ctx.font = "700 96px ui-sans-serif, system-ui";
    ctx.fillText(formatCurrency(netWorth, lang, currency), W / 2, cardY + 200);
    ...
    ctx.fillStyle = "#6b7280";
    ctx.fillText(lang === "th" ? "เดือนนี้" : "This Month", cardX + 60, cardY + 310);

    // Income box
    const boxY = cardY + 340;
    const boxH = 130;
    const boxW = (cardW - 180) / 2;
    ctx.fillStyle = "rgba(16,185,129,0.10)";
    roundRect(ctx, cardX + 60, boxY, boxW, boxH, 24);
    ctx.fill();
    ctx.fillStyle = "#059669";
    ctx.font = "500 24px ui-sans-serif, system-ui";
    ctx.fillText(lang === "th" ? "รายรับ" : "Income", cardX + 90, boxY + 45);
    ctx.font = "700 40px ui-sans-serif, system-ui";
    ctx.fillText(formatCurrency(monthIncome, lang, currency), cardX + 90, boxY + 100);

    // Expense box
    ctx.fillStyle = "rgba(239,68,68,0.10)";
    roundRect(ctx, cardX + 60 + boxW + 60, boxY, boxW, boxH, 24);
    ctx.fill();
    ctx.fillStyle = "#dc2626";
    ctx.font = "500 24px ui-sans-serif, system-ui";
    ctx.fillText(lang === "th" ? "รายจ่าย" : "Expense", cardX + 90 + boxW + 60, boxY + 45);
    ctx.font = "700 40px ui-sans-serif, system-ui";
    ctx.fillText(formatCurrency(monthExpense, lang, currency), cardX + 90 + boxW + 60, boxY + 100);

    // Wallets section
    let y = boxY + boxH + 70;
    ctx.fillStyle = "#6b7280";
    ctx.font = "500 26px ui-sans-serif, system-ui";
    ctx.fillText(lang === "th" ? "กระเป๋าเงิน" : "Wallets", cardX + 60, y);
    y += 30;

    const walletsToShow = walletsWithBalance.slice(0, 6);
    for (const w of walletsToShow) {
      y += 70;
      ctx.fillStyle = "#0f172a";
      ctx.font = "500 34px ui-sans-serif, system-ui, 'Apple Color Emoji', 'Segoe UI Emoji'";
      ctx.textAlign = "left";
      ctx.fillText(`${w.emoji}  ${w.name}`, cardX + 60, y);
      ctx.textAlign = "right";
      ctx.font = "600 34px ui-sans-serif, system-ui";
      ctx.fillStyle = w.balance >= 0 ? "#0f172a" : "#dc2626";
      ctx.fillText(formatCurrency(w.balance, lang, currency), cardX + cardW - 60, y);
    }
    ...
    ctx.fillText(
      lang === "th"
        ? `สรุป ณ ${dateLabel} · v${appConfig.currentVersion}`
        : `Snapshot · ${dateLabel} · v${appConfig.currentVersion}`,
      W / 2,
      H - 60,
    );
  ...
  const handleExportImage = async () => {
    try {
      const dataUrl = renderBackupImage();
      const timestamp = new Date().toISOString().replace(/[:.]/g, "-").slice(0, -5);
      ...
      if (isNative) {
        try {
          ...
          toast.error(lang === "th" ? "ไม่สามารถบันทึกรูปได้" : "Unable to save image");
        }
      } else {
        const a = document.createElement("a");
        a.href = dataUrl;
